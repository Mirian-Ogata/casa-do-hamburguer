...
const Button = ({ title, variant = "default", ...props }: ButtonType) => {
  const buttonVariant = () => {
    if (variant === "default") {
      return "w-full cursor-pointer rounded-md border border-[#C92A0E] bg-[#C92A0E] py-2 text-xs font-bold text-white";
    } else if (variant === "outline") {
      return "w-full cursor-pointer rounded-md border border-[#c92a0e] bg-white py-2 text-xs font-bold text-[#c92a0e]";
